...
object AlarmComparator {
    ...
    val byNextAlarmTime: Comparator<Alarm> = Comparator { a1, a2 ->
    ...
    val byHourMinute: Comparator<Alarm> = Comparator { a1, a2 ->
...
fun main() {
    ...
    alarms.sortWith(AlarmComparator.byHourMinute)
    println("Sorted by hour and minute: $alarms")

    // 상대 시간 기준
    // 1. 켜진 알람을 우선으로 함
    // 2. 현재 시간으로부터 (미래로) 가장 가까운 시간
    alarms.sortWith(AlarmComparator.byNextAlarmTime)
